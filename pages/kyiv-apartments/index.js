...
//         { slug: 'arsenalna', nameUa: 'Арсенальна', nameRu: 'Арсенальная' },
//         { slug: 'teatralna', nameUa: 'Театральна', nameRu: 'Театральная' },
//         { slug: 'majdan-nezalezhnosti', nameUa: 'Майдан Незалежності', nameRu: 'Майдан Независимости' },
//       ].map((metro) => (
...
// {/* ============================================================ */}
// {/* ↓↓↓ НОВЫЙ БЛОК: ССЫЛКИ НА ТИПЫ КВАРТИР (ВСТАВИТЬ СЮДА) ↓↓↓ */}
// {/* ============================================================ */}
...
//       mb: { xs: 2, md: 3 } 
//     }}>
//       {currentLanguage === 'ua' ? 'Детальна інформація по типах квартир Києва' : 'Детальная информация по типам квартир Киева'}
//     </Typography>
    
//     <Grid container spacing={{ xs: 1, sm: 1.5, md: 2 }}>
//       {[
//         { slug: '1-bedroom', nameUa: '1-кімнатні', nameRu: '1-комнатные', icon: '🛏️' },
//         { slug: '2-bedroom', nameUa: '2-кімнатні', nameRu: '2-комнатные', icon: '🛏️' },
//         { slug: '3-bedroom', nameUa: '3-кімнатні', nameRu: '3-комнатные', icon: '🛏️'},
//         { slug: '4-bedroom', nameUa: '4-кімнатні', nameRu: '4-комнатные', icon: '🛏️' },
//       ].map((type) => (
//         <Grid item xs={6} sm={4} md={3} key={type.slug}>
//           <Link href={`/kyiv-apartments/type/${type.slug}`} passHref legacyBehavior>
//             <a style={{ textDecoration: 'none' }}>
//               <Paper sx={{ 
//                 p: { xs: 1, sm: 1.5 }, 
//                 textAlign: 'center', 
...
//                 </Typography>
//                 <Typography variant="body2" fontWeight={500}>
//                   {currentLanguage === 'ua' ? type.nameUa : type.nameRu}
//                 </Typography>
//                 <Typography variant="caption" sx={{ fontSize: '0.65rem', color: 'text.secondary' }}>
//                   {currentLanguage === 'ua' ? 'детальна інформація' : 'детальная информация'}
//                 </Typography>
...
const TRANSLATIONS = {
  ua: {
    ...
    category: "Категорія",
    kyiv: "Київ",
    showAllMetro: "Всі станції",
    showLessMetro: "Показати менше",
    showAllDistricts: "Всі райони",
    showLessDistricts: "Показати менше",
    // SEO контент
    seoTitle1: "Переваги оренди квартири в Києві",
    seoContent1: "Київ — це не лише столиця України, але й місто з унікальною атмосферою, багатою історією та розвиненою інфраструктурою. Оренда квартири в Києві дозволяє вам бути в центрі подій, насолоджуватися культурним життям та мати доступ до всіх зручностей мегаполісу. На нашому сайті представлені квартири різних категорій: від економ-класу до преміум-апартаментів у центрі міста.",
    seoTitle2: "Найкращі райони для оренди квартири в Києві",
  ...
  ru: {
    ...
    category: "Категория",
    kyiv: "Киев",
    showAllMetro: "Все станции",
    showLessMetro: "Показать меньше",
    showAllDistricts: "Все районы",
    showLessDistricts: "Показать меньше",
    // SEO контент
    seoTitle1: "Преимущества аренды квартиры в Киеве",
    seoContent1: "Киев — это не только столица Украины, но и город с уникальной атмосферой, богатой историей и развитой инфраструктурой. Аренда квартиры в Киеве позволяет вам быть в центре событий, наслаждаться культурной жизнью и иметь доступ ко всем удобствам мегаполиса. На нашем сайте представлены квартиры различных категорий: от эконом-класса до премиум-апартаментов в центре города.",
    seoTitle2: "Лучшие районы для аренды квартиры в Киеве",
...
function KyivApartmentsContent({ apartments = [], totalCount = 0, initialFilter = {} }) {
  ...
  // НОВЫЕ СОСТОЯНИЯ ДЛЯ БЛОКОВ С КНОПКАМИ "ПОКАЗАТЬ ВСЕ"
  const [showAllDistrictsBlock, setShowAllDistrictsBlock] = useState(false);
  const [showAllMetroBlock, setShowAllMetroBlock] = useState(false);
  ...
  // НОВЫЕ МАССИВЫ ДЛЯ БЛОКОВ С КНОПКАМИ
  const districtsList = [
    { slug: 'pechersk', nameUa: 'Печерський', nameRu: 'Печерский' },
    { slug: 'obolon', nameUa: 'Оболонський', nameRu: 'Оболоньский' },
    { slug: 'golosiyivsky', nameUa: 'Голосіївський', nameRu: 'Голосеевский' },
  ...
  const metroList = [
    ...
    { slug: 'teatralna', nameUa: 'Театральна', nameRu: 'Театральная' },
    { slug: 'majdan-nezalezhnosti', nameUa: 'Майдан Незалежності', nameRu: 'Майдан Независимости' },
    { slug: 'osokorky', nameUa: 'Осокорки', nameRu: 'Осокорки' },
    { slug: 'chernihivska', nameUa: 'Чернігівська', nameRu: 'Черниговская' },
    { slug: 'darnytsia', nameUa: 'Дарниця', nameRu: 'Дарница' },
  ];

  const typeList = [
    { slug: '1-bedroom', nameUa: '1-кімнатні', nameRu: '1-комнатные', icon: '🛏️', rooms: '1' },
    { slug: '2-bedroom', nameUa: '2-кімнатні', nameRu: '2-комнатные', icon: '🛏️', rooms: '2' },
    { slug: '3-bedroom', nameUa: '3-кімнатні', nameRu: '3-комнатные', icon: '🛏️', rooms: '3' },
    { slug: '4-bedroom', nameUa: '4+ кімнатні', nameRu: '4+ комнатные', icon: '🛏️', rooms: '4+' },
  ];

  // Определяем количество элементов для отображения в блоках
  const getBlockDisplayCount = (type) => {
    if (isMobile) {
      if (type === 'districts') return 6;
      if (type === 'metro') return 6;
      if (type === 'types') return 2;
      return 6;
    }
    if (isTablet) {
      if (type === 'districts') return 8;
      if (type === 'metro') return 8;
      if (type === 'types') return 3;
      return 8;
    }
    if (type === 'districts') return 10;
    if (type === 'metro') return 10;
    if (type === 'types') return 4;
    return 10;
  };

  const displayedDistrictsBlock = showAllDistrictsBlock ? districtsList : districtsList.slice(0, getBlockDisplayCount('districts'));
  const displayedMetroBlock = showAllMetroBlock ? metroList : metroList.slice(0, getBlockDisplayCount('metro'));
  // const displayedTypes = isMobile ? typeList.slice(0, 2) : (isTablet ? typeList.slice(0, 3) : typeList);
  const displayedTypes = typeList;
  return (
    <>
      <Head>
        <title>{getMetaTitle()}</title>
      ...
      <Box component="main" sx={{ minHeight: '100vh', backgroundColor: '#fafafa', position: 'relative' }}>
        ...
        <Container maxWidth="lg" sx={{ py: { xs: 2, sm: 3, md: 4 } }}>
          ...
          {/* ============================================================ */}
          {/* ↓↓↓ ОБНОВЛЕННЫЙ БЛОК: РАЙОНЫ С КНОПКОЙ "ПОКАЗАТЬ ВСЕ" ↓↓↓ */}
          {/* ============================================================ */}
          {isKyiv(activeFilters.city) && (
            <Box sx={{ mt: { xs: 4, md: 6 }, mb: { xs: 2, md: 3 } }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: { xs: 2, md: 3 } }}>
                <Typography variant="h2" sx={{ 
                  ...
                  {currentLanguage === 'ua' ? 'Детальна інформація по районах Києва' : 'Детальная информация по районам Киева'}
                </Typography>
                {districtsList.length > getBlockDisplayCount('districts') && (
                  <Button 
                    size="small" 
                    onClick={() => setShowAllDistrictsBlock(!showAllDistrictsBlock)}
                    endIcon={showAllDistrictsBlock ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                  >
                    {showAllDistrictsBlock 
                      ? (currentLanguage === 'ua' ? t.showLessDistricts : t.showLessDistricts)
                      : (currentLanguage === 'ua' ? t.showAllDistricts : t.showAllDistricts)}
                  </Button>
                )}
              </Box>
              
              <Grid container spacing={{ xs: 1, sm: 1.5, md: 2 }}>
                {displayedDistrictsBlock.map((district) => (
                  <Grid item xs={6} sm={4} md={2} key={district.slug}>
                    <Link href={`/kyiv-apartments/district/${district.slug}`} passHref legacyBehavior>
                      <a style={{ textDecoration: 'none' }}>
                        <Paper sx={{ 
          ...
          {/* ============================================================ */}
          {/* ↓↓↓ ОБНОВЛЕННЫЙ БЛОК: МЕТРО С КНОПКОЙ "ПОКАЗАТЬ ВСЕ" ↓↓↓ */}
          {/* ============================================================ */}
          {isKyiv(activeFilters.city) && (
            <Box sx={{ mt: { xs: 4, md: 6 }, mb: { xs: 2, md: 3 } }}>
              <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mb: { xs: 2, md: 3 } }}>
                ...
                {metroList.length > getBlockDisplayCount('metro') && (
                  <Button 
                    size="small" 
                    onClick={() => setShowAllMetroBlock(!showAllMetroBlock)}
                    endIcon={showAllMetroBlock ? <ExpandLessIcon /> : <ExpandMoreIcon />}
                  >
                    {showAllMetroBlock 
                      ? (currentLanguage === 'ua' ? t.showLessMetro : t.showLessMetro)
                      : (currentLanguage === 'ua' ? t.showAllMetro : t.showAllMetro)}
                  </Button>
                )}
              </Box>
              
              <Grid container spacing={{ xs: 1, sm: 1.5, md: 2 }}>
                {displayedMetroBlock.map((metro) => (
                  <Grid item xs={6} sm={4} md={2} key={metro.slug}>
                    <Link href={`/kyiv-apartments/metro/${metro.slug}`} passHref legacyBehavior>
                      <a style={{ textDecoration: 'none' }}>
                        <Paper sx={{ 
          ...
          {/* ============================================================ */}
          {/* ↓↓↓ ОБНОВЛЕННЫЙ БЛОК: ТИПЫ КВАРТИР (4+ КІМНАТНІ) ↓↓↓ */}
          {/* ============================================================ */}
          {isKyiv(activeFilters.city) && (
            <Box sx={{ mt: { xs: 4, md: 6 }, mb: { xs: 2, md: 3 } }}>
              <Typography variant="h2" sx={{ 
