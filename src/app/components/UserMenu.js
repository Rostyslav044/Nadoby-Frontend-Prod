...
// // Компонент для быстрых ссылок с forwardRef
...
//       return () => document.removeEventListener('mousedown', handleClickOutside);
//     }
//   }, [showEmailDialog, handleClickOutside]);
...
// Поддерживает десктоп (наведение мыши) и мобилку (касание)
// ============================================
const FastLink = memo(React.forwardRef(({ href, children, onClick, ...props }, ref) => {
  const router = useRouter();
  ...
  return (
    <a 
      ...
      href={href}
      onClick={handleClick}
      onMouseEnter={handlePrefetch}    // Десктоп: предзагрузка при наведении мыши
      onTouchStart={handlePrefetch}    // Мобилка: предзагрузка при касании
      onFocus={handlePrefetch}         // Клавиатура: предзагрузка при фокусе
      style={{ textDecoration: 'none', color: 'inherit', width: '100%' }}
      {...props}
    >
      {children}
...
const UserMenu = ({ onClose }) => {
  ...
  // ============================================
  // Функция для загрузки данных пользователя (только 1 раз)
  // ============================================
  const fetchUserData = useCallback(async () => {
    ...
    try {
      if (profile?._id) {
        ...
        setMyListingsCount(listingsResponse.data.count || 0);
      }
      ...
      if (userProfile) {
        ...
        if (favoritesResponse.data.success) {
          setFavoritesCount(favoritesResponse.data.count || 0);
        }
      }
  ...
  }, [profile, isAuthenticated, isDataLoaded]);

  // ============================================
  // ЗАГРУЗКА ДАННЫХ ТОЛЬКО 1 РАЗ ПРИ МОНТИРОВАНИИ
  // ============================================
  useEffect(() => {
    fetchUserData();
    fetchExchangeRates();
  ...
  // ============================================
  // ДОПОЛНИТЕЛЬНАЯ ПРЕДЗАГРУЗКА ДЛЯ МОБИЛКИ (фоновая)
  // Загружаем только самые важные страницы
  // ============================================
  useEffect(() => {
    if (isMobile) {
      // Только самые важные страницы для мобилки
      const mobilePages = [
        '/my-listings',
        '/favorites',
        '/kyiv-apartments'
