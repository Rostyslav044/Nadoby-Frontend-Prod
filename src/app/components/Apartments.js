...
const ApartmentsContent = ({ 
  ...
  initialLoading = false,
  serverTimestamp,
  showAll = false, // 👈 ДОБАВЛЯЕМ ПРОПС
}) => {
  const [allApartments, setAllApartments] = useState([]);
  const [displayedApartments, setDisplayedApartments] = useState([]);
  const [loading, setLoading] = useState(initialLoading);
  const [loadingMore, setLoadingMore] = useState(false);
  const [showAllState, setShowAllState] = useState(showAll); // 👈 ИСПОЛЬЗУЕМ ПРОПС
  const [anchorEl, setAnchorEl] = useState(null);
  const [selectedApartment, setSelectedApartment] = useState(null);
  const [currentUser, setCurrentUser] = useState(null);
  const [snackbar, setSnackbar] = useState({ open: false, message: '', severity: 'info' });
  ...
  // Эффект для обновления отображаемых апартаментов при изменении allApartments или showAllState
  useEffect(() => {
    if (allApartments.length > 0) {
      if (showAllState) {
        setDisplayedApartments(allApartments);
  ...
  const fetchApartments = async () => {
    try {
      ...
      // 👈 ВАЖНО: Если showAll === true, устанавливаем showAllState в true
      if (showAll) {
  ...
  return (
    <Box>
      ...
      <Container sx={{ mt: 1, mb: 1 }}>
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          {/* Можно добавить информацию о количестве */}
        </Box>
      </Container>
      ...
      {/* Кнопка "Показать все" - показываем только если НЕ showAll */}
      {!showAllState && allApartments.length > INITIAL_DISPLAY_COUNT && (
        <Box sx={{ position: 'relative' }}>
          {loadingMore ? (
            <Box sx={{ display: 'flex', justifyContent: 'center', py: 4 }}>
              <CircularProgress />
